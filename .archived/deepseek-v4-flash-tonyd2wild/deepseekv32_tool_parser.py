...
class DeepSeekV32ToolParser(ToolParser):
    ...
    def extract_tool_calls_streaming(
        ...
        if content:
            # Do NOT pass tool_calls here. DeltaMessage declares it as
            # `list[DeltaToolCall] = Field(default_factory=list)` -- not Optional --
            # and the stream is serialised with exclude_unset=True. Passing an
            # explicit empty list therefore marks the field as *set* and emits
            # `"tool_calls": []` on every content delta as soon as the request
            # carries tools (truthy in JS -> client routes text into the
            # tool-call branch and drops the answer). Omitting it restores the
            # documented shape.
