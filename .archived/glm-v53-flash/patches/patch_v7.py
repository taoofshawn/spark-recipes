# Vendored from tonyd2wild/GLM-5.3-Flash-NVFP4-2x-DGX-Spark (docker/patch_v7.py).
# Indexer top-k hardening: the kpool top-k destination was torch.empty and the
# top-k kernels only guarantee the first min(k, valid) entries; short rows
# carried uninitialized pool ids -> bogus token indices -> attention gathers
# uninitialized KV -> NaN lottery. Init to -1 + clamp expanded pool ids.
#
