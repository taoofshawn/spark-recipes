# Vendored from tonyd2wild/GLM-5.3-Flash-NVFP4-2x-DGX-Spark
# (docker/sparse_attn_indexer_kpool_sm121.py) — full-file overlay of vLLM's
# sparse_attn_indexer_kpool.py with the SM121 gate below. This file REPLACES
# the stock file in the image (see patches/Dockerfile step 8).
#
# The persistent_topk kernel (used whenever select_k in 512/1024/2048, which
# GLM-5.3's index_topk=2048/index_kpool=4 always is) sizes its CTA grid to the
# sequence's candidate set; past ~24K context it oversubscribes GB10's SM
# budget and its FilteredTopK fallback requires 128KB smem/block (SM121 has
# ~101KB) -> RuntimeError -> EngineDeadError. Gate it on >= 78 SMs so small-SM
# parts take the existing top_k_per_row_decode fallback instead.
#
