...
class _DrafterCompilationConfigView:
    """Read-only view of the target's CompilationConfig with drafter-private
    cudagraph sizes.

    Installed on the drafter's ``CudagraphDispatcher`` only (never on the
    shared ``VllmConfig``), so the target keeps its spec-decode-rounded
    ``cudagraph_capture_sizes`` (multiples of 1 + k) while the DSpark draft
    graphs dispatch on a list keyed by *requests*, not target tokens.
    Every attribute that is not overridden here is delegated to the real
    config object.
    """
    ...
    def __init__(self, base: Any, capture_sizes: list[int]) -> None:
        ...
        # ``compile_sizes`` is only consulted by the dispatcher to validate
        # that inductor compile sizes are not changed by padding; that check
        # is about the *target* token sizes and must not fire on the
        # drafter's request-sized buckets.
...
class DSparkProposer(SpecDecodeBaseProposer):
    ...
    def __init__(
        ...
        # Optional drafter-private cudagraph size list (VLLM_DSPARK_DRAFT_CAPTURE_SIZES).
        # Off (None) => the drafter shares the target's rounded sizes exactly as before.
        self._draft_capture_sizes = self._read_draft_capture_sizes(self.max_batch_size)
        if self._draft_capture_sizes is not None:
            self.cudagraph_dispatcher.compilation_config = _DrafterCompilationConfigView(
                self.compilation_config, self._draft_capture_sizes
            )
            logger.info(
                "DSpark drafter-private cudagraph capture sizes enabled: %s "
                "(target sizes stay %s). Draft graphs are keyed by request "
                "count; a batch of B requests now runs B x %d draft tokens.",
                self._draft_capture_sizes,
    ...
    @staticmethod
    def _read_draft_capture_sizes(max_batch_size: int) -> list[int] | None:
        """Parse VLLM_DSPARK_DRAFT_CAPTURE_SIZES.

        ""/0/off  -> None (feature off, byte-identical legacy behaviour)
        1/on/auto -> powers of two below max_num_seqs plus max_num_seqs
                     (e.g. max_num_seqs=4 -> [1, 2, 4])
        "a,b,c"   -> explicit request-count buckets; values above
                     max_num_seqs are dropped and max_num_seqs is always
                     appended so every batch has a bucket.
        """
        raw = os.getenv("VLLM_DSPARK_DRAFT_CAPTURE_SIZES", "").strip().lower()
        ...
        for item in raw.split(","):
            ...
            try:
                ...
                raise ValueError(
                    "VLLM_DSPARK_DRAFT_CAPTURE_SIZES must be 0/1/auto or a "
                    f"comma-separated list of positive integers, got {raw!r}"
                ) from exc
            if value <= 0:
                raise ValueError(
                    "VLLM_DSPARK_DRAFT_CAPTURE_SIZES entries must be positive, "
                    f"got {value}"
                )
            if value > max_batch_size:
                logger.warning(
                    "VLLM_DSPARK_DRAFT_CAPTURE_SIZES entry %d exceeds "
                    "max_num_seqs=%d and is ignored.",
                    value,
                    max_batch_size,
                )
                continue
    ...
    def dummy_run(
        ...
        if (
            ...
            # The runner only calls us with the *target's* capture sizes
            # (multiples of 1 + k, clipped to max_num_seqs), which would
            # never capture the smaller drafter-private buckets. Capture
            # every missing drafter bucket here, largest first, with at
            # least one eager warmup per bucket so lazy kernel/autotune
            # state is created outside of graph capture.
            num_warmups = max(
                1, int(getattr(self.compilation_config, "cudagraph_num_of_warmups", 0))
            )
            for size in sorted(self._draft_capture_sizes, reverse=True):
    ...
    def _draft_graph_is_captured(self, batch_size: int) -> bool:
        runner = self._draft_graph_runner
        if runner is None:
            return True
        ...
        if cudagraph_mode != CUDAGraphMode.PIECEWISE:
            # Not a graph-dispatched size; nothing to capture.
            return True
        entries = getattr(runner, "concrete_cudagraph_entries", None)
        if not entries or batch_descriptor not in entries:
            return False
        return entries[batch_descriptor].cudagraph is not None
