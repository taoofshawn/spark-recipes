...
# PATCH(stop-in-reasoning) ported from the aiden/tonyd2wild overlay to vLLM
# main: vLLM's v1 detokenizer matches client stop strings against the whole
# output stream. With think-in-prompt templates generation starts inside
# `` thinking`` and chain-of-thought naturally restates phrases like
# ``Question:``; a client stop firing mid-reasoning yields ``content: null``
# (a silent answer loss). The guard arms when the request's last prompt token
# is the reasoning-start marker and keeps all stop strings dormant until the
# reasoning-end marker appears. Opt out: VLLM_SUPPRESS_STOPS_IN_REASONING=0.
from abc import ABC, abstractmethod
...
class BaseIncrementalDetokenizer(IncrementalDetokenizer, ABC):
    def __init__(self, request: EngineCoreRequest):
        ...
        # opening token (prompt-side  thinking, as DeepSeek-V4/Qwen3 templates
    ...
    def update(self, new_token_ids: list[int], stop_terminated: bool) -> str | None:
        ...
        if self.stop and self.num_output_tokens() > self.min_tokens:
            ...
            if not self._reasoning_stop_guard or self._reasoning_closed:
