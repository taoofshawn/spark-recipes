...
bool launch_sparse_mla_decode_dsv4(ModelType mt, int num_heads, int topk, int page_block_size,
  ...
  // PATCH(dsv4-pagesize): accept 256-token pages in addition to the original
  // 64. vLLM main's DSV4 compressed-SWA KV groups require pages divisible by
  // the 128-token compression ratio, so 64-token pages cannot form a common
  // block size ("No common block size for 64"). The kernel's addressing is
  // generic in PAGE_BLOCK_SIZE (idx/section_pbs decomposition + runtime
  // stride_kv_block), so a 256 instantiation is layout-correct.
  if (page_block_size != 64 && page_block_size != 256) return false;
  if (num_splits <= 0) return false;
#define DSV4_DISPATCH_PSIZE(H, K, PS)                                                       \
  if (num_heads == (H) && topk == (K) && page_block_size == (PS)) {                          \
    return launch_decode_dsv4_impl<ModelType::DSV4, (H), (K), PS>(                           \
...
#define DSV4_DISPATCH(H, K) \
  DSV4_DISPATCH_PSIZE(H, K, 64) \
  DSV4_DISPATCH_PSIZE(H, K, 256)
  DSV4_DISPATCH(8, 128)
  DSV4_DISPATCH(8, 192)
  DSV4_DISPATCH(8, 256)
  DSV4_DISPATCH(8, 512)
